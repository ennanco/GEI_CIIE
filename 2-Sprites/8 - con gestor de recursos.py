"""
Ejemplo 8: Gestor de Recursos
---------------------------

Este ejemplo es parte de una serie de tutoriales incrementales para crear un videojuego.
Construye sobre el Ejemplo 7 (Sprites con Saltos) añadiendo un gestor de recursos para
optimizar la carga y reutilización de imágenes y archivos.

   - Gestión centralizada de recursos (Singleton)
   - Carga única de imágenes y archivos (patrón Flyweight)
...
* Carga lazy (bajo demanda) de recursos
* Cache de recursos
* Métodos de clase
...
ARCHIVO_SPRITE = 'Jugador.png'  # Archivo del sprite del jugador
ARCHIVO_COORDENADAS = 'coordJugador.txt'  # Archivo de coordenadas
...
class GestorRecursos:
    """
    Clase que gestiona la carga y almacenamiento de recursos del juego.
    Implementa el patrón Singleton para asegurar una única instancia.
    Además imple
    """
    _recursos = {}  # Diccionario para almacenar los recursos cargados
            
    @classmethod
    def CargarImagen(cls, nombre, colorkey=None):
        """
        Carga una imagen desde el directorio 'images'.
        Si la imagen ya está cargada, la devuelve del caché.
        """
        # Si el nombre de archivo está entre los recursos ya cargados
        if nombre in cls._recursos:
            return cls._recursos[nombre]
        ...
        cls._recursos[nombre] = imagen
        return imagen

    @classmethod
    def CargarArchivoCoordenadas(cls, nombre):
        """
        Carga un archivo de coordenadas desde el directorio 'images'.
        Si el archivo ya está cargado, lo devuelve del caché.
        """
        # Si el nombre de archivo está entre los recursos ya cargados
        if nombre in cls._recursos:
            return cls._recursos[nombre]
...
class Jugador(pygame.sprite.Sprite):
    ...
    def __init__(self, imagen, coordenadas, posicion):
        super().__init__()
        # Cargamos la hoja de sprites usando el gestor de recursos
        self.hoja = GestorRecursos.CargarImagen(imagen, -1)
        ...
        # Cargamos las coordenadas usando el gestor de recursos
    ...
    def update(self, tiempo):
        ...
        if self.movimiento == IZQUIERDA:
            ...
            self.mirando = IZQUIERDA
            self.posicionx -= VELOCIDAD_JUGADOR * tiempo # posición en el mundo
            self.rect.left = self.posicionx # posición en pantalla  
        ...
        if self.numPostura == SPRITE_SALTANDO:
            ...
            # Si llegamos tope de altura, terminamos el salto
...
def main():
    ...
    pygame.display.set_caption('Ejemplo 8: Gestor de Recursos')
